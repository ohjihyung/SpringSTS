...
public class DatabaseConnectController {
		 ...
		 *   3. 데이터소스 설정 
		 *     - 애플리케이션이 데이터베이스에 접근하기 위한 추상화된 연결을 
		 *       제공하는 역할을 한다.
		 *       
		 *        
		 ...
		 *               	<property name = "username" value="ojh95"/>
		 ...
		 *      
		 *   5. 스프링 JDBC
		 *      - SQL로만 데이터베이스를쉽게 처리할 수 있도록 도와주는 JDBCTemplate 클래스를 제공한다.
		 *      
		 ...
		 *        
		 *             
		 *     ***기업면접에서 orm은 뭐 사용하셧나요?
		 *         네 저는 ibatis를 사용했습니다. 또는 네 저는 mybatis를 사용했습니다.
		 *     
		 *   
		 *   
		 *   데이터베이스 설계 라인이 정확하게 설계되어 있는 상태에서는 JPA가 가독성이 좋다만 
		 *    우리는 mybatis를 사용
		 *    
		 *    
		 *   
		 ...
		 *       
		 *       1) 의존관계 정의
		 *       - pom.xml에 mybatis 설정을 위한 의존관계 등록
		 *       > mybatis
		 *       > mybatis - spring
		 *       > spring-jdbc
		 *       > commons-dbcp2
		 ...
		 *            > 사용할 자바빈즈 클래스들의 패키지명이 포함된 경로 부분을 제외하고 
		 *            클래스 명과 매핑 된 별칭으로 사용할 수 있게 셋팅
		 *            
		 *            > '_'로 연결되어 있는 변수명들을 카멜기법을 이용해 'bo_no'를 'boNo'로 변환되서 
		 *              사용할 수 있도록 셋팅 
		 *              
		 *       
		 *       4) 마이바티스 구현
