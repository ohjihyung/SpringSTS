...
public class PaginationInfoVO<T> {
	...
	private int currentPage; 			  //현재 패이지
	...
	// PaginationInfoVO 객체를 만들 때, 한 페이지당 게시글 수와 페이지 블록 수를 원하는 값으로 초기화 가능 
	...
	public void setCurrentPage(int currentPage) {
		this.currentPage = currentPage;   // 3페이지 
		endRow = currentPage * screenSize; //  마지막 행이 30 
		startRow = endRow - (screenSize - 1); // 시작행 =  21 
	...
	public String getPagingHTML() {
		...
		for(int i = startPage; i<= (endPage< totalPage ? endPage : totalPage); i++) {
			if(i == currentPage) {
				  html.append("<li class='page-item'><font class='page-link'>" + i + "</font></li>");
