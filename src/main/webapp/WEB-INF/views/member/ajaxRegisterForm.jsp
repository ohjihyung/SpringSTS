...
$(function(){
	...
	}); //end of registerbtn01
	//2) URL 경로 상의 여려개의 경로 변수값을 @PathVariable 어노테이션을 지정하여 여러개의 문자열 매개변수로 처리한다 
	...
	//8) 중첩된 객체 타입 JSON 요청 데이터를
	//@RequestBody 어노테이션을 지정하여 중첩된 자바빈즈 매개변수로 처리한다.
	...
	registerBtn09.on("click", function(){
		...
		var userObject= {
				...
				password : passwordVal,
				cardList : [
					{no : "12345", validMonth : "20221127"},
					{no : "54321", validMonth : "20221027"}
				]
...
<body>
	...
	<p>2) URL 경로 상의 여려개의 경로 변수값을 @PathVariable 어노테이션을 지정하여 여러개의 문자열 매개변수로 처리한다 </p>
	
	<hr/>
	<form>
		<p>userId : <input type="text" name ="userId" id ="userId" value = "hongjd"></p>
	...
	<p>4) 객체 타입의 JSON 여청 데이터는 문자열 매개변수로 처리할 수 없다.</p>
	...
	<p>6) 객체 타입의 JSON 요청 데이터를 @PathVariable 어노테이션을 지정한 문자열 매개변수와 <br/>
		  @RequestBody 어노테이션을 지정한 자바빈즈 매개변수로 처리한다.
	</p>
	<button id = "registerBtn06">registerBtn06</button>


	<p>7) 객체 배열 타입의 JSON 요청 데이터를 자바빈즈 요소를 가진 리스트 컬랙션 
		매개변수에 @RequestBody 어노테이션을 지정하여 처리한다
	</p>
	<button id = "registerBtn07">registerBtn07</button>
	
	<p>
	8) 중첩된 객체 타입 JSON 요청 데이터를 @RequestBody 어노테이션을 지정하여 중첩된 자바빈즈 매개변수로 처리한다.
	</p>
	<button id = "registerBtn08">registerBtn08</button>
	
	<p>
	9) 중첩된 객체 타입 JSON 요청 데이터를 @RequestBody 어노테이션을 지정하여 중첩된 자바빈즈 매개변수로 처리한다.
	</p>
	<button id = "registerBtn09">registerBtn09</button>
	
	<p>
		@RequestBody를 사용하면 남기는 파라미터가 받는 파라미터의 객체 속성으로 포함되어 있다면 객체를 자동으로 생성해준다.<br/>
		Member,List제네릭, Map 제네릭 등등 
	</p>
